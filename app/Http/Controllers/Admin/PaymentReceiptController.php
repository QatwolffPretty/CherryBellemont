...
class PaymentReceiptController extends Controller
{
    ...
    public function approve(Request $request, PaymentReceipt $paymentReceipt, OrderNotifier $notifier, AdminNotificationService $adminNotifier, AccountingPostingService $accounting): RedirectResponse { [$order, $receipt] = DB::transaction(function()use($paymentReceipt,$request){$r=PaymentReceipt::lockForUpdate()->findOrFail($paymentReceipt->id);$o=$r->order()->lockForUpdate()->firstOrFail();abort_unless($r->status==='pending' && $o->payment_status!=='paid',409);$r->update(['status'=>'approved','reviewed_by'=>$request->user()->id,'reviewed_at'=>now()]);$o->update(['payment_status'=>'paid']);return [$o, $r];});try{$accounting->postPaidOrder($order);}catch(\Throwable $exception){Log::error('Confirmed DuitNow payment could not be posted to accounting.',['order_number'=>$order->order_number,'exception_class'=>$exception::class]);}$notifier->send($order,'payment_approved');$adminNotifier->send('duitnow_payment_approved',['order'=>$order,'reviewerName'=>$request->user()->name,'approvedAt'=>$receipt->reviewed_at]);return back()->with('success','Receipt approved and payment marked paid.'); }
    ...
    public function download(PaymentReceipt $paymentReceipt) { $disk = Storage::disk($paymentReceipt->storageDisk()); abort_unless($disk->exists($paymentReceipt->path),404); return $disk->download($paymentReceipt->path,$paymentReceipt->original_filename ?: 'receipt'); }
}
