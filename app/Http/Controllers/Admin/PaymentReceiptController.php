...
class PaymentReceiptController extends Controller
{
    ...
    public function approve(Request $request, PaymentReceipt $paymentReceipt, OrderNotifier $notifier): RedirectResponse { $order=DB::transaction(function()use($paymentReceipt,$request){$r=PaymentReceipt::lockForUpdate()->findOrFail($paymentReceipt->id);$o=$r->order()->lockForUpdate()->firstOrFail();abort_unless($r->status==='pending' && $o->payment_status!=='paid',409);$r->update(['status'=>'approved','reviewed_by'=>$request->user()->id,'reviewed_at'=>now()]);$o->update(['payment_status'=>'paid']);return $o;});$notifier->send($order,'payment_approved');return back()->with('success','Receipt approved and payment marked paid.'); }
