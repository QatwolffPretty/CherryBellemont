...
class OrderController extends Controller
{
    ...
    public function show(Request $request, Order $order): View { $this->authorizeOrder($request, $order); return view('orders.show', ['order' => $order->load('items')]); }
    ...
    public function guestShow(Order $order, string $token): View { $this->authorizeGuest($order, $token); return view('orders.show', ['order' => $order->load('items'), 'token' => $token]); }
