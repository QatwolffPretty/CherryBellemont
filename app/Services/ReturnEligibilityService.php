...
class ReturnEligibilityService
{
    public function canRequest(Order $order): bool
    {
        return $order->payment_status === 'paid'
            ...
            && $order->delivered_at
            && $order->delivered_at->greaterThanOrEqualTo(now()->subDays((int) config('store.returns.return_window_days', 14)))
    ...
    public function eligibleItems(Order $order)
    {
        ...
        if ($order->delivered_at->lt(Carbon::now()->subDays((int) config('store.returns.return_window_days', 14)))) return collect();
