    ...
    <section class="mx-auto max-w-6xl px-6 py-16">
        ...
        @else
                ...
                @foreach($lines as $line)
                    <article class="grid gap-5 border border-cream/15 p-5 sm:grid-cols-[7rem_1fr_auto]">
                        <div class="aspect-[3/4] overflow-hidden bg-wine-deep">
                            @if($line['product']->image_path)<img class="h-full w-full object-cover" src="{{ asset('storage/' . $line['product']->image_path) }}" alt="{{ $line['product']->name }}">@else<div class="flex h-full items-center justify-center text-gold/70">CB</div>@endif
                        </div>
                        <div>
                            <h2 class="text-2xl">{{ $line['product']->name }}</h2><p class="mt-1 text-gold">RM {{ number_format($line['unit_price'] / 100, 2) }}</p>
                            <div class="mt-4 flex flex-wrap items-center gap-2">
                                <form method="POST" action="{{ route('cart.update', $line['product']) }}">@csrf @method('PATCH')<input type="hidden" name="quantity" value="{{ max(1, $line['quantity'] - 1) }}"><button class="nav-link" type="submit" @disabled($line['quantity'] <= 1)>−</button></form>
                                <form class="flex items-center gap-2" method="POST" action="{{ route('cart.update', $line['product']) }}">@csrf @method('PATCH')<input class="field w-20" type="number" name="quantity" min="1" max="{{ $line['product']->stock }}" value="{{ $line['quantity'] }}"><button class="nav-link" type="submit">Update</button></form>
                                <form method="POST" action="{{ route('cart.update', $line['product']) }}">@csrf @method('PATCH')<input type="hidden" name="quantity" value="{{ $line['quantity'] + 1 }}"><button class="nav-link" type="submit" @disabled($line['quantity'] >= $line['product']->stock)>+</button></form>
                            </div>
                            @error('quantity')<p class="mt-2 text-gold">{{ $message }}</p>@enderror
                        </div>
                        <div class="flex flex-col items-end justify-between"><p class="text-xl text-gold">RM {{ number_format($line['line_total'] / 100, 2) }}</p><form method="POST" action="{{ route('cart.destroy', $line['product']) }}">@csrf @method('DELETE')<button class="nav-link" type="submit">Remove</button></form></div>
                    </article>
                @endforeach
            </div>
            ...
            <div class="mt-10 ml-auto max-w-sm border-t border-gold/40 pt-6">
                ...
                <p class="mt-3 flex justify-between"><span>Discount</span><span class="text-gold">−RM {{ number_format($couponSummary['discount_cents'] / 100, 2) }}</span></p>
